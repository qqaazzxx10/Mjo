...
#التصميم
logo = pyfiglet.figlet_format('*      DYTHON      *')
print(X+logo)
print('  ')
print(A+'═'*60)
print('  ')
...
try:
    ...
    api_id = int(input(A+"APP ID "+X))
    print('  ')
    api_hash = input(A+"APP HASH "+X)
    print('  ')
    bot_token = input(A+"TOKEN "+X)
    print('  ')
    DEVLOO = input(A+"Controller ID "+X)
    print('  ')
    MAX_ACCOUNTS = int(input(A+"NUMPERS "+X))
    print('  ')
    user_bot = input(A+"USER BOT "+X)
    print('  ')
    id_bot = int(input(A+"BOT ID "+X))
    print('  ')
...
async def start(event):
    ...
    if sender.id == DEVELOPER_ID:
        ...
        [Button.inline('NOTE ; ملاحظة', 'no3te')],    
         [Button.inline('༺ 𝐃𝐘𝐓𝐇𝐎𝐍 𝐒𝐎𝐔𝐑𝐂𝐄 𖠤 ༻', 'button0')]
        ]
        await bot.send_message(chat, '''**اهلا بك في سورس Dython**''', buttons=buttons)
...
async def back(event):
        ...
        await event.edit("""**اهلا بك في سورس Dython**""", buttons=buttons)
...
#-------------- 6
@bot.on(events.CallbackQuery(pattern='no3te'))
async def callback(event):
    await event.edit("**vVv**")

@bot.on(events.CallbackQuery(pattern='button0'))
async def callback(event):
    await event.edit("**• المطور @kxk_x •**")
